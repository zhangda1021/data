...
set nonzero(i,j)
loop(i$(ord(i) le 30),
loop(j$((ord(j)=ord(i)+30) or (ord(j)=66)),
nonzero(i,j)=yes;
););
loop(i$((ord(i) ge 31) and (ord(i) le 60)),
loop(j$(((ord(j) ge 1) and (ord(j) le 30)) or (ord(j)=63) or (ord(j)=65) or (ord(j)=70) or (ord(j)=71) or (ord(j)=72) or (ord(j)=73)),
nonzero(i,j)=yes;
););
nonzero("57","74")=yes;
loop(i$((ord(i) ge 61) and (ord(i) le 62)),
loop(j$((ord(j) ge 1) and (ord(j) le 30)),
nonzero(i,j)=yes;
););
loop(i$(ord(i)=63),
loop(j$((ord(j)=61) or (ord(j)=65) or (ord(j)=70) or (ord(j)=71) or (ord(j)=62)),
nonzero(i,j)=yes;
););
loop(i$(ord(i)=64),
...
loop(i$(((ord(i) ge 70) and (ord(i) le 71)) or (ord(i)=73)),
loop(j$(((ord(j) ge 31) and (ord(j) le 60)) or (ord(j)=63)),
nonzero(i,j)=yes;
););
loop(i$(ord(i)=72),
...
*);
*);
finalsam.fx(r,i,j)$(not nonzero(i,j))=0;
*);
);
